const amf = require('amf-client-js');
const models = amf.model.domain;

beforeAll(() => {
    amf.AMF.init();
});

test('webapi builder test', () => {
    const stringDataType = 'http://www.w3.org/2001/XMLSchema#string';

    const api = new models.WebApi()
        .withName('Music Service API')
        .withVersion('v1')
        .withContentType(['application/json'])
        .withAccepts(['application/json']);

    // create EndPoint /me
    const me = api.withEndPoint('/me').withName('current-user');
    // add get operation
    const meGet = me.withOperation('get').withDescription('Get Current User\'s Profile');
    // add a 200 response of type string to the get operation
    meGet.withResponse('200')
        .withPayload()
        .withMediaType('application/json')
        .withScalarSchema('schema')
        .withDataType(stringDataType);


    // create EndPoint /me/playlists
    const playlists = api.withEndPoint('/me/playlists').withName('current-user-playlists');
    const playlistsGet = playlists.withOperation('get')
        .withDescription('Get a List of Current User\'s Playlists');
    playlistsGet.withResponse('200')
        .withPayload()
        .withMediaType('application/json')
        .withScalarSchema('schema')
        .withDataType(stringDataType);


    // create EndPoint /albums/{id}
    const album = api.withEndPoint('/albums/{id}').withName('albums');
    // add {id} parameter to albums endpoint
    album.withParameter('id')
        .withDescription('The album ID')
        .withBinding('path')
        .withRequired(true)
        .withScalarSchema('music service Album ID')
        .withDataType(stringDataType);
    // create a request and a query parameter to add to the get operation
    const marketRequest = new models.Request();
    const marketQueryParam = marketRequest.withQueryParameter('market')
        .withRequired(false)
        .withName('Market')
        .withDescription(' The market (an ISO 3166-1 alpha-2 country code)');
    marketQueryParam
        .withBinding('query')
        .withScalarSchema('Market')
        .withDataType(stringDataType);
    // add get operation to albums/{id} endpoint
    album.withOperation('get')
        .withDescription('Get an Album')
        .withRequest(marketRequest)
        .withResponse('200')
        .withPayload()
        .withMediaType('application/json')
        .withScalarSchema('schema')
        .withDataType(stringDataType);


    // create EndPoint /albums with a query parameter of a list of album IDs and a string response
    const albums = api.withEndPoint('/albums').withName('several-albums');
    const albumsRequest = new models.Request();
    albumsRequest.withQueryParameter('ids')
        .withBinding('query')
        .withRequired(true)
        .withName('album-ids')
        .withDescription('A comma-separated list of IDs')
        .withScalarSchema('displayName')
        .withDataType(stringDataType);
    albums.withOperation('get')
        .withDescription('Get Several Albums')
        .withRequest(albumsRequest)
        .withResponse('200')
        .withPayload()
        .withMediaType('application/json')
        .withScalarSchema('schema')
        .withDataType(stringDataType);


    expect(api.endPoints.length).toEqual(4);

    const model = new amf.model.document.Document();
    model.withEncodes(api);
    const renderer = new amf.Raml10Renderer();
    renderer.generateString(model).then(m => console.log(m));
})
