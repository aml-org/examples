import {AMLConfiguration, Api, Document, OASConfiguration, PipelineId, WebApi} from "amf-client-js";
...
describe("Semantic Extensions", () => {
    ...
    const INVALID_EXTENDED_API_SPEC = "file://src/test/resources/examples/semantic/invalid-api.oas30.yaml"
    ...
    it("can read applied extensions from an Endpoint and Response Object", async () => {
        ...
        const endpointIsDeprecated = api.endPoints[0].graph().containsProperty("http://a.ml/vocabularies/apiContract#deprecated")
        ...
        */
        const deprecatedExtension = api.endPoints[0].graph().getObjectByProperty("http://a.ml/vocabularies/apiContract#deprecated")[0]
        const replaceForValue = deprecatedExtension.graph().scalarByProperty("http://a.ml/vocabularies/data#replaceFor")[0] as string
        expect(replaceForValue).to.equal("v2/paginated")

        // Check for the page-size extension in the second endpoint
        const pageSize = api.endPoints[1]
            .operations.find(x => x.method.value() == "get")
            .responses[0]
            .graph().scalarByProperty("http://a.ml/vocabularies/apiContract#pageSize")[0]
