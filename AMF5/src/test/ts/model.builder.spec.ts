...
describe("Model Builder", () => {
    ...
    it("renders built model", async () => {
        const golden = fileSystem.readFileSync(RENDER_GOLDEN_FILE, {encoding: UTF8})
        const model: BaseUnit = buildModel()
        const client = RAMLConfiguration.RAML10().createClient()
    ...
    function buildModel(): BaseUnit {
        ...
        const meGet: Operation = meEndpoint.withOperation("get").withDescription("Get Current User's Profile")
        meGet
        ...
        const playlistsEndpoint: EndPoint = api.withEndPoint("/me/playlists").withName("current-user-playlists")
        const playlistsGet: Operation = playlistsEndpoint.withOperation("get").withDescription("Get a List of Current User's Playlists")
        playlistsGet
            .withResponse(HTTP_STATUS_OK)
            .withPayload()
            .withMediaType(APPLICATION_JSON)
