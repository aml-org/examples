import {
    amf,
    AMFClient, AMFConfiguration,
    AMFDocumentResult, AMFResult, Document, ValidationMode, WebApi,
    WebAPIConfiguration
} from "amf-client-js";
...
describe("Shape payload validation", () => {
    ...
    it("validates shape correctly", async () => {
        ...
        const result: AMFDocumentResult = await client.parseDocument("file://resources/examples/simple-api.raml")
        ...
        const validator = validatorFactory.createFor(userSchema, APPLICATION_JSON, ValidationMode.StrictValidationMode)
